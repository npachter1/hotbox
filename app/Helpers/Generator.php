...
class Generator
{
    ...
    /* geenerate a unique suite id */
    public static function uniqueLocation(){
      
        $exist = Location::pluck('id')->all();
		$code = rand(100000,999999);                                            // 6-digit random integer
			while(in_array($code,$exist)) $code = rand(10000000,99999999);      // Ensure generation is unique
    ...
    /* generate a barcode if one was not inputted */
    public static function genBarcode($params=[]){
        

        $_tag = data_get($params, 'metrc_tag', 'NONTHC');
        $_cat = data_get($params, 'category_id', '12345');
   
        /* [mounth][day][year] [last 6 chars of tag] [product_category.id]-[random] */
        return date('mdy',time()).' '.substr($_tag, -6).' '.$_cat.'-'.rand(444,4444);
