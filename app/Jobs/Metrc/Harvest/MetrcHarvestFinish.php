...
class MetrcHarvestFinish extends MetrcBaseJob
{
    ...
    public function handle(MetrcHarvestApi $api)
    {
        ...
        if (!$this->test_mode) {
            ...
            $http_code = $api->finishHarvest($this->user, $jsonItems);
            ...
            // check that the harvest now has a FinishedDate in Metrc
            foreach($this->items as $harvest) {
                ...
                if (!$metrc_item->FinishedDate) {
                    ...
                    throw new Exception('Harvest Not Finished Successfully');
                }
            }
        }
        foreach($this->items as $item) {
