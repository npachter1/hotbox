...
class MetrcItemDelete extends MetrcBaseJob
{
    ...
    public function handle(MetrcRequestService $api)
    {
        ...
        if (!$this->test_mode) {
            foreach($this->items as $item) {
                $http_code = $api->deleteItems($this->user, $item->metrc_id);
                ...
                if ($http_code !== 200) {
                    ...
                    $item->save();
                    throw new Exception('Delete Items Not Successful, returned HTTP code '.$http_code);
                }
