...
class MetrcRoomUpdate extends MetrcBaseJob
{
    /**
     * Execute the job.
     */
    public function handle(MetrcRequestService $api)
    {
        ...
        if (!$this->test_mode) {
            ...
            $http_code = $api->updateRooms($this->user, $jsonItems);
            ...
            if ($http_code !== 200) {
                ...
                throw new Exception('Update Rooms Not Successful, returned HTTP code '.$http_code);
            }
