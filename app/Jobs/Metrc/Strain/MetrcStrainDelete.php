...
class MetrcStrainDelete extends MetrcBaseJob
{
    ...
    public function handle(MetrcRequestService $api)
    {
        ...
        if (!$this->test_mode) {
            foreach($this->items as $item) {
                $http_code = $api->deleteStrains($this->user, $item->metrc_id);
                ...
                if ($http_code !== 200) {
                    ...
                    $item->save();
                    throw new Exception('Delete Strains Not Successful, returned HTTP code '.$http_code);
                }

                // Metrc Strain was deleted successfully, now delete ours:
                $item->deleted_by = $this->user->id;
