 ...
 */
class AuthSeederProduction extends Seeder
{
    ...
    public function run()
    {
        ...
        DB::table('auth_locations')->insert([
            [
                ...
                'status'            => 'activated',
                'type'              => 'dispensary',
                'licensenum'        => null,                                    // bgood case study TODO remove from seeder..
                'thumb'             => null,
        ...
        /* initial addresses */
        $defDispAdr = Addressbook::create([
            'location_id'       => 12345,
            'type'              => 'location',
            'name'              => 'Dispensary Address',
        ...
        Location::where('id',12345)->update(['addressbook_id'=>$defDispAdr->id]);
        Location::where('id',67890)->update(['addressbook_id'=>$defGrowAdr->id]);
        
        
        
        
        
        /* initial data for dispensary location (super user is assigned to it) */
        Auth::guard('api')->setUser(User::find(self::USER_SUPER_ID)); // Authenticate passsed user (with passed location_id from contruct) - this will get us access to globally scoped data sets (ie: Helpers/LocationScope)
        
        /* initial tax group and rate */
        $rate = new TaxRate;
        ...
        $taxa = new Tax;
        $taxa->location_id = 12345;
        $taxa->nature_type = 'recreational';
        $taxa->save();
        
        $taxa->rates()->sync([$rate->id]);
        
        $taxb = new Tax;
        $taxb->location_id = 12345;
        $taxb->nature_type = 'medical';
        $taxb->save();
        
        $taxb->rates()->sync([$rate->id]);
