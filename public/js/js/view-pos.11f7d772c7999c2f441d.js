...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    closeOrderItem: function closeOrderItem() {
      ...
      this.orderItemModal = !this.orderItemModal;
      this.$nextTick(function () {
        return _this5.$refs.scanAdd.focus();
    ...
    completePayment: function completePayment(pay) {
      ...
      axios.post('/api/v1/admin/dispensary/sales/' + this.order.id + '/payment', pay).then(function (response) {
        _this8.receiptBarcode = _this8.order.order_number;

        _this8.printReceipt(_this8.order.id);

        _this8.$store.dispatch('pos/getCurrentDrawer'); // re-load current users drawer as a sale has been tendered


        _this8.order = new _models_Sale__WEBPACK_IMPORTED_MODULE_1__["default"]().withDefaults(_this8.schema); // reset with a new sale object, with defaults, in case budtender wants to ring up a new transaction.

        _this8.order.customer_id = _this8.customer.id;
        _this8.order.drawer_id = _this8.drawer.id;
    ...
    voidOrder: function voidOrder() {
      ...
      this.$swal.fire({
        ...
        if (result.value) {
          _this9.isProcessing = true;

          if (!_this9.order || !_this9.order.id) {
            _this9.$store.commit('pos/setCustomerOrder', null);

            _this9.$router.push({
              name: 'terminal-queue'
            });
          } else {
            axios.get('/api/v1/admin/dispensary/sales/' + _this9.order.id + '/void').then(function (response) {
              ...
              _this9.$announcer(error.response);
            });
          }
        } else {//
        }
    ...
    reQueue: function reQueue() {
      ...
      axios.get('/api/v1/admin/dispensary/customersqueue/add/' + this.customer.id).then(function (response) {
        _this10.$store.commit('pos/setCustomerOrder', null);
        ...
        if (error.response.status === 422 && error.response.data.message.indexOf('already') > 0) {
          ...
          _this10.$announcer(error.response);
        }
      });
    },
    _updateOrder: function _updateOrder() {
      ...
      this.order.save().then(function (response) {
        _this11.order = response;
        _this11.isProcessing = false;
      })["catch"](function (error) {
        _this11.isDirty = true;

        _this11.$announcer(error.response);
          ...
          _c(
            "div",
            {
              style: {
                display:
                  _vm.item.unit_of_measure === "ea" &&
                  ((_vm.item.product || {}).category || {}).equivalency_type ===
                    "flower"
                    ? "block"
                    : "inline"
              }
