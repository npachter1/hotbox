            ...
            <div class="card top-blue-bdr">
                <div class="card-header">
                    <div v-if="indexView" class="">
                        <router-link v-if="indexView.with_add && !isEditPage" :to="{name: indexView.name+'_create'}" tag="a" class="float-right mt-0 mb-3 mr-4">
                            <span class="btn-label"><i class="hotbox-icon hotbox-icon-e-add"></i></span> New {{ indexView.resource_name || indexView.name | ucwords }}
                        </router-link>    
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div v-if="" class="stats">
                        <router-link v-if="!isEditPage" :to="{name:($route.name==='analytics') ? 'location_index' : 'analytics'}" tag="a" class=""><i class="ti-angle-left"></i> Back to Dashboard</router-link>
