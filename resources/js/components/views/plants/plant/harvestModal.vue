<template>
    <div v-if="schema" class="col-12">
        <form class="modal-form" id="harvestform" @submit.prevent="submit">
        <fieldset>
        <h3 class="mb-0">Harvest {{ ids.length }} {{ 'Plant'| pluralize(ids.length)}}</h3>
       ...
       <div class="form-row">
        ...
        <div class="col-6">
          ...
          <div class="form-group">
            <select class="form-control" name="drying_room" :data-vv-as="'drying room'" v-model="newItem.drying_room_id" v-validate="'required'">
       ...
       <div class="form-row">
        ...
        <div class="col-12 clearfix mt-2 text-center">
            <button class="btn btn-info" type="submit">Harvest Plants</button>
        </div>
        </fieldset>
        </form>
...
<script>
    ...
    export default {
        ...
        methods: {
             submit(e) {
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        ...
                        this.$swal.fire({
                        title: 'Are you sure?',
                        text: 'This will harvest '+this.ids.length+' plants',
                        type: 'warning',
                        ...
                        if(result.value){
                            this.isProcessing = true;
                                axios.post('/api/v1/admin/grow/plants/harvest',this.newItem).then(response =>{
            ...
            initializeWeights(type) {
                this.newItem[type] = {};
                if (this.sources)
                    this.sources.forEach((item) => {
                        this.newItem[type][item.id] = 0;
                });
            },
            setWeight(type, id) {
                if (!this.newItem[type.target.name])
                    this.initializeWeights(type.target.name);
