                    ...
                    <table role="table" class="table b-table">
                        ...
                        <tbody>
                            <tr v-for="(item,ind) in batch">
                                <td><img :src="(item.thumb) ? item.thumb : schema.model.thumb" class="responsive" width="45"> {{item.name}}</td>
            ...
            <transition name="bo-slide">
                <div v-if="tab=='archive'">
                    ...
                    <div class="block-announce warning mt-4 mb-4">
                        ...
                        <p>This will remove from any view or report, but would still be used if linked to anything.  Please confirm these are the Categories you would like to archive.</p>
                    </div>
                    ...
                    <table role="table" class="table b-table table-striped">
                        ...
                        <tbody>
                            <tr v-for="(item,ind) in batch" :class="{'table-success':inSet(item.id,batch_ids)}">
                                <td width="90%"><img :src="(item.thumb) ? item.thumb : schema.model.thumb" class="responsive" width="45"> {{item.name}}</td>
