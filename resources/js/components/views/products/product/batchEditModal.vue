                    ...
                    <table role="table" class="table b-table">
                        ...
                        <tbody>
                            <tr v-for="(item,ind) in batch">
                                <td><img :src="getProductImageUrl({product:item})" class="responsive" width="45"> {{item.name}}</td>
                                <td v-for="(fsch,fkey) in batchFields">
                                    ...
                                    <form-number v-if="fsch.type=='number'" v-model="item[fkey].override" :schema="fsch" :hideLabel="true" />
            ...
            <transition name="bo-slide">
                <div v-if="tab=='archive'">
                    ...
                    <div class="block-announce warning mt-4 mb-4">
                        ...
                        <p>This will remove from any view or report, but would still be used if linked to anything.  Please confirm these are the Products you would like to archive.</p>
                    </div>
                    ...
                    <table role="table" class="table b-table table-striped">
                        ...
                        <tbody>
                            <tr v-for="(item,ind) in batchCanArchive" :class="{'table-success':inSet(item.id,batch_ids)}">
                                <td width="90%"><img :src="(item.public_img) ? item.public_img : '/images/none.jpg'" class="responsive" width="45"> {{item.name}}</td>
                    ...
                    <div class="col-12 clearfix mt-2 text-center">
                        <auto-save type="save" :state="batchState" :disabled="batchCanArchive.length===0" @autoSave="saveBatchData('list')"></auto-save>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            sortByVendor() {
                this.batch.sort((a,b)=>{
                    let aName = (a?.last_received?.vendor?.name ? a.last_received.vendor.name : a.last_received.addressbook_id.toString());
                    let bName = (b?.last_received?.vendor?.name ? b.last_received.vendor.name : b.last_received.addressbook_id.toString());
        ...
        computed: {
            ...
            batchCanArchive() {
                return this.batch.filter(e=>{
                    return ((e.inv_meta===null) ||((e.inv_meta.onhand||0)<=0) )
                });
            },
